...
_FF_Ext_Copy/
├── extension/
│   ├── manifest.json               # Firefox MV2 manifest (AMO upload source)
│   ├── manifest.chromium.json      # Chromium MV3 manifest (test build only)
...
│   ├── assets/mathml2omml.xsl      # MathML→OMML XSLT transformer
│   ├── mathjax/tex-mml-chtml.js    # MathJax (LaTeX→MathML)
│   ├── wasm/tex_to_mathml.wasm     # Rust WASM converter (subset TeX→MathML)
│   └── icons/                      # Extension icons
├── examples/                       # Real-world offline fixtures (Gemini/ChatGPT)
├── tests/                          # Playwright tests + Word paste checks
├── tools/                          # Build/probe scripts (xpi, wasm, payload capture)
├── docs/                           # Documentation
├── plans/                          # Active plans
├── plans_completed/                # Completed plans / historical notes
└── artifacts/                     # Ad-hoc evidence bundles (ignored by git)
```

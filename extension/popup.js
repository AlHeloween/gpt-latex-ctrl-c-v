(() => {
  ...
  // Language list from TWP (269 languages)
  const LANGUAGE_OPTIONS = `<option value="ab">Abkhaz</option>
...
<option value="sjn">Elvish (Sindarin)</option>
...
<option value="zu">Zulu</option>`;
  ...
  function populateLanguageDropdowns() {
    for (let i = 1; i <= 5; i++) {
      const select = document.getElementById(`popupTargetLanguage${i}`);
      if (select) {
        select.innerHTML = '<option value="">None</option>' + LANGUAGE_OPTIONS;
      }
    }
  }
  ...
  async function loadSettings() {
    try {
      ...
      for (let i = 0; i < 5; i++) {
        const el = document.getElementById(`popupTargetLanguage${i + 1}`);
  ...
  async function saveSettings() {
    try {
      ...
      config.translation.targetLanguages = [
        document.getElementById("popupTargetLanguage1").value,
        document.getElementById("popupTargetLanguage2").value,
        document.getElementById("popupTargetLanguage3").value,
        document.getElementById("popupTargetLanguage4").value,
        document.getElementById("popupTargetLanguage5").value,
      ];
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // Handle changes
    document.getElementById("popupTranslationEnabled").addEventListener("change", saveSettings);
    for (let i = 1; i <= 5; i++) {
      document.getElementById(`popupTargetLanguage${i}`).addEventListener("change", saveSettings);
    }

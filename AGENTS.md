...
- Extension sources: `content-script.js`, `background.js`, `constants.js`, `manifest.json` (Firefox MV2), and `manifest.chromium.json` (Chromium MV3 test build).
- Assets: `assets/`, `mathjax/`, `icons/`; automation in `tests/`; helpers in `tools/`.

## Build, Test, and Development Commands (uv)
- `uv sync` - sync environment.
- `uv run python tests/test_automated.py --browser chromium` - smoke suite (DOM-proved readiness; no "network idle").
...
- Tests: Python follows PEP 8; fixtures `tests/test_*.html`, runners `tests/test_*.py`.
